...
class MyBinaryClassifier(nn.Module):
    ...
    def __init__(self, embedding_dim, vocab_size, hidden_dim, num_heads) -> None:
        ...
        self.lstm = nn.LSTM(input_size=embedding_dim, hidden_size=hidden_dim, batch_first=True)  # expects (batch_size, sequence_length, input_size) and outputs (batch_size, sequence_length, hidden_size)
        """
        output_sequence, (h_n, c_n) = self.lstm(x)
        
            output_sequence (batch_size, sequence_length, hidden_size): output sequence of the LSTM for each time step. a[i, j, :] contains the hidden state at time step j for the sample i in the batch.
            
            h_n (num_layers * num_directions, batch_size, hidden_size): the hidden state of the LSTM at the last time step.
            c_n (num_layers * num_directions, batch_size, hidden_size): the cell state of the LSTM at the last time step.
    ...
    def forward(self, x):
        ...
        # -- LSTM; generates (batch_size, sequence_length, embedding_dim)
        output_sequence, (h_n, c_n) = self.lstm(x)  # (1024, 20, 100) -> (1024, 20, 64)
        
        #-- pooling
        # x = output_sequence.mean(dim=1)  # avg pooling across sequence_length => generates (B, embedding_dim)
        x = output_sequence[:,-1,:]  # use last time_step output as the feature pooling 
