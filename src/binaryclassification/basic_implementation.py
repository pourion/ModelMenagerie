...
batch_size = 1024
sequence_length = 20
x = torch.randint(0, vocab_size, (batch_size, sequence_length))  # shape (batch_size, sequence_lenght)
...
class MyBinaryClassifier(nn.Module):
    ...
    def __init__(self, embedding_dim, vocab_size, hidden_dim, num_heads) -> None:
        ...
        self.embedding = nn.Embedding(num_embeddings=vocab_size, embedding_dim=embedding_dim)  # lookup table shape (vocab_size, embedding_dim)
        
        self.attention = nn.MultiheadAttention(embed_dim=embedding_dim, num_heads=num_heads, batch_first=True)  # view each embedding as a sequence and use self attention
    ...
    def forward(self, x):
        #-- receives x.shape = (batch_size, sequence_length)
...
model = MyBinaryClassifier(embedding_dim=embedding_dim, vocab_size=vocab_size, hidden_dim=hidden_dim, num_heads=num_heads)
